        ...
        { 
          ...
          obj = JSON.parse(response); 
            ...
            if(obj.data.length<12){
               $("#imagenes_bus").html("No se encontrÃ³ una imagen");
            }
            else
            {
            ...
            htmlB = '<div class="col-md-12">'
                      ...
                      +'</div>'
                        ...
                        +'<div class="col-md-12">'
                          ...
                          +'<div class="col-md-6" style="text-align: center; padding-top:15px;">'
                            +'<a onclick="buscar_imagen(2);" style="background-color:#26A8FF; width:250px;" class="waves-effect waves-light btn">MÃ¡s imÃ¡genes</a>'
                          +'</div>'
               ...
               toastr["error"]("IntÃ©ntalo de nuevo mÃ¡s tarde", "ERROR");
             }
        });
}
...
function enviarBotonComenta2(opcion){
  ...
  if(typeof redesAdd[0]=="undefined"){
    ...
    toastr["warning"]("Instagram necesita mÃ­nimo una imÃ¡gen.");
    $('body').addClass('loaded');
	  ...
	  if(parseInt(timeP)>parseInt(timeR) && (Date.parse(timePFechaParse)==Date.parse(timeRFechaParse)) || (Date.parse(timePFechaParse)>Date.parse(timeRFechaParse))){
		...
		for(var i=0; i<redesAdd.length; i++){
			...
			if(redesAdd[i]['id'].substr(redesAdd[i]['id'].length-2, redesAdd[i]['id'].length)=="tw"){
				...
				userTempName = redesAdd[i]['name'];
				redMsg = 'instagram';
			    toastr["success"]("Para poder lanzar el mensaje programado de instagram, necesitas descargar la APP mÃ³vil de bamboostr Disponible en App Store y Play Store.");
            }
...
function cerrarImage2(image,url){
  ga('send', 'event', 'Eliminar ImÃ¡gen', 'click', 'Eliminar ImÃ¡gen');
...
function editImage2(image,url){
  ga('send', 'event', 'Editar ImÃ¡gen', 'click', 'Editar ImÃ¡gen');
...
function add2(numImageDiv, imagenGrande){
  ga('send', 'event', 'Agregar ImÃ¡gen Recomendada', 'click', 'Agregar ImÃ¡gen Recomendada');
  console.log("Agregar ImÃ¡gen Recomendada");
