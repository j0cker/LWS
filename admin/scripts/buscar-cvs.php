...
$buscar = $_GET["buscar"];
$query = $conn->query("SELECT * FROM cvs WHERE id LIKE '%".$buscar."%' OR nombreCompleto LIKE '%".$buscar."%' OR estado LIKE '%".$buscar."%' OR tipoTiempo LIKE '%".$buscar."%' OR fecha LIKE '%".$buscar."%'") OR die("Error: ".mysqli_error($conn));
if($query->num_rows>0){
  $c=0;
  $array_id = array();
  $categoria = array();
